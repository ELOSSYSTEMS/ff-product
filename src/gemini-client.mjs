  ...
  const sizeGuideValidationRules =
    isSizeGuideSlot(slot)
      ? [
          ...
          "- fail if the product is not centered or not fully visible",
          "- fail if the background is a flat blank white canvas instead of a minimalist studio tabletop scene with a subtle wall/table horizon, depth, and soft shadow",
          "- fail if the product is placed on a tray, wooden riser, plinth, pedestal, block, stand, prop, book, or decorative object instead of directly on the tabletop",
          "- fail if the vertical height guide is missing or not on the right side of the product",
          "- fail if the horizontal width guide is missing or not below the product",
          "- fail if measurement guide lines use arrowheads instead of simple perpendicular end ticks",
          "- fail if measurement guide lines are thick black, white, decorative, or ruler-like instead of thin gray technical lines",
          `- fail unless the height label is present, legible Hebrew, not rotated, and contains exactly גובה plus ${heightCm} plus ס״מ; it may be one line or two clean lines`,
          `- fail unless the width label is present, legible Hebrew, not rotated, and contains exactly רוחב plus ${widthCm} plus ס״מ; it may be one line or two clean lines`,
          "- fail if any English labels, small explanatory text, or extra text appears",
          "- fail only if important content is cropped, illegible, or unusably close to the image edges in a Shopify mobile PDP gallery",
          ...
          "- fail if full-length ruler lines appear along the image side or bottom",
          "- fail if rulers, measuring tapes, yardsticks, sticky notes, handwritten notes, acrylic blocks, plaques, cameras, books, hands, people, props, comparison objects, room clutter, lifestyle decor, extra decorations, or real-world scale objects appear",
          "- fail if the scene reads as a lifestyle scale scene instead of a clean mobile PDP catalog measurement graphic"
        ]
          .filter(Boolean)
      : [];
