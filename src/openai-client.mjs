...
export function buildCopyPrompt(input, attempt = 0, rejectedStems = []) {
  ...
  return [
    ...
    "- the size-guide image prompt must forbid rotated labels, English labels, extra small text, physical rulers, measuring tapes, yardsticks, sticky notes, handwritten notes, acrylic blocks, plaques, cameras, books, hands, people, trays, platforms, risers, props, comparison objects, room clutter, and lifestyle decor",
...
export function buildImageRetryPrompt(
  ...
  if (["size-guide", "mobile-size-guide"].includes(slot)) {
    ...
    slotSpecificRules.push(
      ...
      "Do not rotate Hebrew labels. Hebrew labels may be split into two clean lines if needed.",
